...
inpath = os.path.join('All_Watershed_streams')
...
print allrecords[0][np.squeeze(np.where(justfields=='rivsysname'.lower()))-1]
...
for cr in allrecords:
    riversysname_only.append(cr[np.squeeze(np.where(justfields=='riversysname'.lower()))-1])
